...
var check = function () {
    if ($('.container .equations input:checked').val() == undefined) {
        alert("You need to select at least one equation.");
        return 0;
    }
    if ($('.container .mode input[type="radio"]:checked').val() == undefined) {
        alert("You need to select a game mode.");
        return 0;
    }
    $('.container .equations input:checked').each(function (i) {
        Game.transformList.push($(this).val());
    });
    Game.moder = $('.container .mode input[type="radio"]:checked').val();
    Game.parentChoose = ($('.container .mode input[type="checkbox"]:checked').val() == undefined) ? false : true;
    startGame();
