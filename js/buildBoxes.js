...
function setBoxes() {
    ...
    for (a = 3; a < games.length + 2; a++) {
        ...
        $(".container > .num > div:nth-child(" + a + ") > div > a > .slide > h3").html(games[a - 2].shortDesc);
        $(".container > .num > div:nth-child(" + a + ") > div > a > img.bkgrnd").attr("data-original", "projects/" + games[a - 2].href + "image.png");
        ...
        if(games[a-2].release == 1) //beta
            $(".container > .num > div:nth-child(" + a + ") > div").append("<img class='beta' src='resources/images/beta.png'>");
		if(games[a-2].release == 0) { //coming soon
            $(".container > .num > div:nth-child(" + a + ") > div").append("<img class='beta' src='resources/images/comingSoon.png'>");
			$(".container > .num > div:nth-child(" + a + ") a").prop("href", "#");
		}
		if(games[a-2].release != 0) { //add Play button
			$(".container > .num > div:nth-child(" + a + ") .slide > section").html("Play");
			$(".container > .num > div:nth-child(" + a + ") .slide > section").css("background-color", "#4CAF50");
			$(".container > .num > div:nth-child(" + a + ") .slide > section").css("box-shadow", "0px 2px 5px rgba(0, 0, 0, .5)");
			$(".container > .num > div:nth-child(" + a + ") .slide > section").css("width", "20%");
		}
    }
    //featured box
	var a = featured.game;
    ...
    $(".container > .num > .featured > div > a > .slide > h3").html(games[a].shortDesc);
    $(".container > .num > .featured > div > a > img").attr("data-original", "projects/" + games[a].href + "image.png");
...
function GameSmallCSS() {
	$(".gameBox").css("transition", "all 1s");
	$(".gameWindow").css("height", "884px");
...
function setToGame(box, num) {
	$(".gameBox.gameWindow").attr("src", "projects/game.html#" + (num - 2));
}
...
function checkGame() {
	if(window.location.hash) {
		...
		$(".gameAnim").css("transition", "all 0s");
			...
			setTimeout(function() {
				...
				setTimeout(function() {
					...
					$(".gameAnim").css("height", "100%");
					$(".gameAnim").css("left", "calc(50% - 550px)");
					$(".gameAnim").css("top", window.scrollY);
		...
		setTimeout(function() {
			...
			setToGame($(".gameWindow"), a[0]);
			$(".gameWindow").css("height", "100%");
			$(".gameX").css("height", "50px");
			$(".gameX").css("right", "calc(50% - 550px)");
			$(".gameBox").css("display", "block");
			$(".gameBox").css("transition", "all .5s");
			setTimeout(function() {
				$(".gameBox").css("transition", "all .5s");
				$(".gameAnim").css("transition", "all .5s");
				$(".gameBox").css("opacity", "1");
				setTimeout(function() {$(".gameAnim").css("opacity", "0")}, 500);
			}, 100);
		}, 1000);
	}
	else {
...
/*$(window).load(function() {
	$("iframe.gameBox").on("beforeunload", function() {
		var location = this.contentWindow.location.href;
		console.log("play: " + location.substr(location.length - 10, 10));
		if(location.substr(location.length - 10, 10) == "index.html") {
			gameFull();
		}
	});
});*/